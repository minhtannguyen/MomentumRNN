...
from orthogonal import OrthogonalRNN, OrthogonalMomentumRNN
...
parser.add_argument("-m", "--mode",
                    choices=["exprnn", "dtriv", "cayley", "lstm", "mlstm", "mdtriv", "nlstm", "alstm"],
                    default="dtriv",
                    type=str)
...
if args.mode == "exprnn":
    mode = "static"
    param = expm
elif args.mode == "dtriv" or args.mode == "mdtriv":
...
class Model(nn.Module):
    def __init__(self, hidden_size):
        ...
        if args.mode == "lstm":
            ...
            self.rnn = MomentumLSTMCell(n_input, hidden_size, mu=args.mu, epsilon=args.epsilon, fg_init=-4.0)
        elif args.mode == "alstm":
            self.rnn = AdamLSTMCell(n_input, hidden_size, mu=args.mu, epsilon=args.epsilon, mus=args.mus, fg_init=-4.0)
        elif args.mode == "nlstm":
            self.rnn = NesterovLSTMCell(n_input, hidden_size, epsilon=args.epsilon, fg_init=-4.0)
    ...
    def forward(self, inputs):
        if isinstance(self.rnn, OrthogonalRNN) or isinstance(self.rnn, OrthogonalMomentumRNN):
