...
class OrthogonalRNN(nn.Module):
    ...
    def default_hidden(self, input):
        return input.new_zeros(input.size(0), self.hidden_size, requires_grad=True)
...
class OrthogonalMomentumRNN(nn.Module):
    def __init__(self, input_size, hidden_size, initializer_skew, mode, param, mu, epsilon):
        super(OrthogonalMomentumRNN, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.recurrent_kernel = Orthogonal(hidden_size, hidden_size, initializer_skew, mode, param=param)
        self.input_kernel = nn.Linear(in_features=self.input_size, out_features=self.hidden_size, bias=False)
    ...
    def forward(self, input, hidden, v):
        vy = self.mu * v + self.epsilon * self.input_kernel(input)
        out = vy + self.recurrent_kernel(hidden)
        out = self.nonlinearity(out)

        return out, out, vy
    
class OrthogonalAdamRNN(nn.Module):
    def __init__(self, input_size, hidden_size, initializer_skew, mode, param, mu, epsilon, mus):
        super(OrthogonalAdamRNN, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.recurrent_kernel = Orthogonal(hidden_size, hidden_size, initializer_skew, mode, param=param)
        self.input_kernel = nn.Linear(in_features=self.input_size, out_features=self.hidden_size, bias=False)
    ...
    def forward(self, input, hidden, v, s):
        
        grad_val = self.input_kernel(input)
        vy = self.mu * v + self.epsilon * grad_val
        sy = self.mus * s + (1.0 - self.mus) * (grad_val * grad_val)
        out = vy/torch.sqrt(sy + 1e-16) + self.recurrent_kernel(hidden)
        
        out = self.nonlinearity(out)

        return out, out, vy, sy
    
    
class OrthogonalNesterovRNN(nn.Module):
    def __init__(self, input_size, hidden_size, initializer_skew, mode, param, epsilon):
        super(OrthogonalNesterovRNN, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.recurrent_kernel = Orthogonal(hidden_size, hidden_size, initializer_skew, mode, param=param)
        self.input_kernel = nn.Linear(in_features=self.input_size, out_features=self.hidden_size, bias=False)
    ...
    def forward(self, input, hidden, v, k):
        
        vy = (k-1.0)/(k+2.0) * v + self.epsilon * self.input_kernel(input)
        out = vy + self.recurrent_kernel(hidden)
        out = self.nonlinearity(out)
        
#         vy = self.recurrent_kernel(hidden) - self.epsilon * self.input_kernel(input)
#         out = vy + (k-1.0)/(k+2.0) * (vy - v)   
#         out = self.nonlinearity(out)
